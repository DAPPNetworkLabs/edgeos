...
namespace wasm_evm
{
  ...
  void Processor::sload()
  {
    ...
    if (ctx->s.stack_error) return throw_stack();
    uint256_t loaded = loadkv(ctx->callee.get_address(), k);
  ...
  void Processor::sstore()
  {
    ...
    // Load current value
    uint256_t current_value = loadkv(ctx->callee.get_address(), k);

    // Charge gas
    auto original_value = transaction.find_original(ctx->callee.get_address(), k);
    bool error = process_sstore_gas(original_value, current_value, v);
    if (error) return;

    // Store
    storekv(ctx->callee.get_address(), k, v);
  }
  ...
  void Processor::call()
  {
    ...
    auto success_cb = [&, p_ctx, size_out, off_out](const std::vector<uint8_t>& output, const uint256_t& sub_gas_used) {
      ...
      auto bytes_to_copy = std::min(static_cast<unsigned long>(size_out), output.size());
      if (bytes_to_copy > 0) {
  ...
  void Processor::selfdestruct()
  {
    ...
    if (balance > 0) {
      ...
      {
        auto account = get_account_by_address(ctx->callee.get_address());
        account->balance = 0;
        set_account(ctx->callee.get_address(), *account);
      }
