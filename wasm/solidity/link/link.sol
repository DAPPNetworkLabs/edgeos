...
abstract contract link {
    ...
    uint64 public next_incoming_message_id;

    uint256 public last_incoming_message_block_num;
     ...
     */
    function getBatch(uint64 batch_id)
        public
        view
        returns (
            uint256 id,
    ...
    function pushInboundMessage(uint256 id, bytes memory _message) external {
        ...
        last_incoming_message_block_num = block.number;
        // TODO: check this logic
        next_incoming_message_id = uint64(id + 1);
    }
    ...
    function pushMessage(bytes memory _message) internal {
        Message memory message =
            Message(available_message_id, _message, block.number, false);
    ...
    function pushReceipt(uint256 id, bytes memory _message) internal {
        ...
        Message memory message =
            Message(receipt_id, _message, block.number, true);
        batches[available_batch_id] = message;
